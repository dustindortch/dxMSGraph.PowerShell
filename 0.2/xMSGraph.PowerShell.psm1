...
function Connect-Graph
{
    ...
    Process
    {
        ...
        if ($UserName -and $Password) {
            $TokenParams.add("UserName", $UserName) | Out-Null
            $BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($Password)
            $PlaintextPwd = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR)
            $TokenParams.add("Password", $PlaintextPwd) | Out-Null
        } else {
            $TokenParams.add("ForcePromptSignIn", $True) | Out-Null
            $TokenParams.add("RedirectUri", $Script:RedirectUri) | Out-Null
        }

        $Token = Get-ADALAccessToken @TokenParams
